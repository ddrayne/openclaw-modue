...
function registerClawFull(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      // Compact header — 60px: emoji (60px) + status + elapsed
      layer.addRow((row) => {
        row.setSize(228, 60).setBackgroundColor(COLORS.header);
        row.addColumn((col) => {
          col.setSize(48, 60);
      ...
      // Content row — 788px: massive wrapping text area (was 668 + 180 header)
      layer.addRow((row) => {
        row.setSize(228, 788).setBackgroundColor(COLORS.bg);
        row.addDisplayText('content', (t) =>
          t.setText('Waiting for activity...')
            .setColor(COLORS.text)
            .setFontSize(26)
