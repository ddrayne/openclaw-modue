...
function registerClawStream(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      layer.addRow((row) => {
        row.setSize(114, 76).setBackgroundColor(COLORS.bg);
...
function registerClawFull(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      // Compact header — 24px (renders 48px): emoji (20) + status (68) + elapsed (26)
      layer.addRow((row) => {
        row.setSize(114, 24).setBackgroundColor(COLORS.header);
        row.addColumn((col) => {
          col.setSize(20, 24);
      ...
      // Content row — 426px (renders 852px): THE MAIN EVENT. Big readable text.
      layer.addRow((row) => {
