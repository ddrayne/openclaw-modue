...
function registerChannels(configuration, storage, log) {
  ...
  configuration
    ...
    .registerOnInitializeHandler((instance) => {
      ...
      // Refresh channel list periodically + when connectivity flips,
      // never per-notify. Per-notify fan-out (called once per agent token
      // delta during streaming) used to queue thousands of in-flight RPCs
      // and OOM the Modue platform under load.
      const refresh = () => {
        if (inflight) return inflight;
        if (!conn._client) return null;
        inflight = conn.rpc('channels.status', {})
          .then((r) => { renderChannels(dc, r); return r; })
          .catch(() => { dc['ch-list']?.set('offline'); })
      ...
      // Single notify subscriber; only refetch on actual connectivity change.
      unsub = conn.onChange((c) => {
        if (c.connected !== lastConnected) {
          lastConnected = c.connected;
          if (c.connected) refresh();
        }
      });
    })
    .registerOnConfigurationChangeHandler(() => {})
...
function registerClawInfo(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      layer.addRow((row) => {
        row.setSize(240, 50).setBackgroundColor(COLORS.bg);
      ...
      // Bottom detail
      layer.addRow((row) => {
        row.setSize(240, 90).setBackgroundColor(COLORS.bg);
    ...
    .registerOnInitializeHandler((instance) => {
      ...
      fetchCost();
      costInterval = setInterval(fetchCost, 30000); // refresh every 30s
      ...
      unsub = bindDisplay(conn, dc, (c, d) => {
        ...
        // Priority 2: Active (thinking/talking/working)
        ...
        // Priority 3: Idle — show cost or ready
