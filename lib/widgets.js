...
function registerClawStream(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      layer.addRow((row) => {
        row.setSize(114, 92).setBackgroundColor(COLORS.bg);
    ...
    .registerOnInitializeHandler((instance) => {
      ...
      unsub = bindDisplay(conn, dc, (c, d) => {
        d.status?.set(c.statusPhrase);
        const text = c.displayText || 'Waiting...';
        const src = formatSource(c);
        d.content?.set(src ? text + '\n' + src : text);
      });
    })
    .registerOnConfigurationChangeHandler(() => {})
    .registerOnDeactivateHandler(() => {
...
function registerClawLive(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      layer.addRow((row) => {
        row.setSize(114, 206).setBackgroundColor(COLORS.bg);
        row.addDisplayText('content', (t) =>
    ...
    .registerOnInitializeHandler((instance) => {
      ...
      unsub = bindDisplay(conn, dc, (c, d) => {
        ...
        const text = c.displayText || 'Waiting for activity...';
        const src = formatSource(c);
        const ctrl = formatControls(c);
        const suffix = [src, ctrl].filter(Boolean).join(' \u2022 ');
        d.content?.set(suffix ? text + '\n\n' + suffix : text);
      });
    })
    .registerOnConfigurationChangeHandler(() => {})
    .registerOnDeactivateHandler(() => {
...
function registerClawFull(configuration, storage, log) {
  ...
  configuration
    ...
    .addLayer((layer) => {
      ...
      // Content row — 444px (renders 888px): all the space, no footer
      layer.addRow((row) => {
        row.setSize(114, 444).setBackgroundColor(COLORS.bg);
        row.addDisplayText('content', (t) =>
          t.setText('Waiting for activity...')
            .setColor(COLORS.text)
            .setFontSize(24)
    ...
    .registerOnInitializeHandler((instance) => {
      ...
      unsub = bindDisplay(conn, dc, (c, d) => {
        d.status?.set(c.statusPhrase);
        d.elapsed?.set(c.statusDetail || '');
        // Append source to content when available
        const text = c.displayText || 'Waiting for activity...';
        const src = formatSource(c);
        const ctrl = formatControls(c);
        const suffix = [src, ctrl].filter(Boolean).join(' \u2022 ');
