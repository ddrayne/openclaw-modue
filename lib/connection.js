  ...
  _wireEvents() {
    ...
    c.on('connection', (info) => {
      ...
      if (info.connected) {
        ...
        // Prime the bridge plugin so agent tools can broadcast to us
        this._client.rpc('modue.bridge.ping', {}).catch(() => {
          this._log.info('modue.bridge.ping not available (plugin may not be loaded)');
        });
