...
class Connection {
  ...
  connect() {
    ...
    this._log.info(`connect() url=${url} token=${token ? token.length + ' chars' : 'EMPTY'}`);
  ...
  getLedColors(numLeds, animFrame) {
    ...
    if (this.ledOverride?.pattern) {
      const result = this._patternColors(n, frame, this.ledOverride);
      if (frame % 10 === 0) this._log.info(`LED pattern frame=${frame} first=${result[0]}`);
      return result;
    }
  ...
  _wireEvents() {
    ...
    c.on('modue.leds.set', (payload) => {
      this._log.info(`LED set: ${JSON.stringify(payload)}`);
      if (payload?.colors) {
    ...
    c.on('modue.leds.pattern', (payload) => {
      this._log.info(`LED pattern: ${JSON.stringify(payload)}`);
      if (payload?.pattern) {
        this.ledOverride = {
