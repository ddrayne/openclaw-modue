...
class Connection {
  ...
  _handleThinking(evt) {
    ...
    if (delta) {
      // Delta = incremental new text, append it
      this._appendText(delta);
    } else if (evt.data?.thinking) {
      // Accumulated thinking text — replace, don't append
      this._rawBuffer = evt.data.thinking;
  ...
  _handleAssistant(evt) {
    ...
    if (delta) {
      ...
      // Accumulated text — replace, don't append
      this._rawBuffer = evt.data.text;
      this.currentText = this._rawBuffer;
      this.displayText = this._extractDisplayText(this._rawBuffer);
    }
  }
