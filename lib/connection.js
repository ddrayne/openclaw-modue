...
class Connection {
  ...
  /**
   * Extract the most useful text for the small display.
   * Strips markdown formatting, grabs the last complete sentence or chunk.
   */
  _extractDisplayText(raw) {
    ...
    if (sentenceBreak >= 0 && sentenceBreak < DISPLAY_TEXT_LENGTH - 20) {
      return tail.slice(sentenceBreak + 2).trim();
    }
    // Fallback: break at last space
    const lastSpace = tail.lastIndexOf(' ', DISPLAY_TEXT_LENGTH - 10);
    if (lastSpace > 20) {
      return '\u2026' + tail.slice(lastSpace).trim();
    }
    return tail;
  }
