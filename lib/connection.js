...
class Connection {
  ...
  _wireEvents() {
    ...
    c.on('connection', (info) => {
      ...
      if (info.connected) {
        ...
        // Ping the bridge so it captures broadcast() — retry until it works
        this._pingBridge();
  ...
  // ---------------------------------------------------------------------------
  // Internal — bridge ping with retry
  // ---------------------------------------------------------------------------
  ...
  _pingBridge(attempt) {
    ...
    this._client.rpc('modue.bridge.ping', {}).then(() => {
      this._log.info('modue bridge connected');
    }).catch(() => {
      if (attempt < 5) {
        setTimeout(() => this._pingBridge(attempt + 1), 2000 * attempt);
      } else {
        this._log.warn('modue bridge not available after 5 attempts');
      }
    });
  }
